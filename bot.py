...
async def process_name(message: Message, state: FSMContext):
    ...
    if not recipes:
        await message.answer("😕 Ничего не найдено. Попробуйте другое название.", reply_markup=kb.main_menu)
        await state.clear()
        return
    ...
    for r in recipes[:5]:
        ...
        buttons.append([types.InlineKeyboardButton(text=f"🍽 {title}", callback_data=f"show_{r['id']}")])
    
    await message.answer(f"✅ Найдено {len(recipes)} рецептов:", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=buttons))
    await state.clear()
...
async def process_ingredients(message: Message, state: FSMContext):
    ...
    if not recipes:
        await message.answer("😕 Ничего не найдено. Попробуйте другие ингредиенты.", reply_markup=kb.main_menu)
        await state.clear()
        return
...
async def show_favorites(message: Message):
    ...
    if not favorites:
        await message.answer("❤️ У вас пока нет избранных рецептов.", parse_mode="Markdown")
        return
    await message.answer("❤️ *Ваши избранные рецепты:*", parse_mode="Markdown", reply_markup=kb.favorites_keyboard(favorites))
...
async def show_recipe(message: Message, recipe):
    ...
    if recipe.get('image'):
        try:
            await message.answer_photo(photo=recipe['image'], caption=text, parse_mode="Markdown", reply_markup=kb.recipe_keyboard(recipe['id'], title_display, is_favorite))
        except:
            await message.answer(text, parse_mode="Markdown", reply_markup=kb.recipe_keyboard(recipe['id'], title_display, is_favorite))
    else:
        await message.answer(text, parse_mode="Markdown", reply_markup=kb.recipe_keyboard(recipe['id'], title_display, is_favorite))
...
async def callback_add_fav(callback: CallbackQuery):
    ...
    if success:
        ...
        await callback.message.edit_reply_markup(reply_markup=kb.recipe_keyboard(recipe_id, recipe_name, True))
...
async def callback_clear_favorites(callback: CallbackQuery):
    ...
    await callback.message.answer("🗑 *Вы уверены?*\n\nЭто действие удалит ВСЕ рецепты из избранного.", parse_mode="Markdown", reply_markup=confirm_keyboard)
...
async def callback_confirm_clear(callback: CallbackQuery):
    ...
    if success:
        await callback.message.edit_text("✅ *Избранное очищено!*", parse_mode="Markdown")
    else:
        await callback.message.edit_text("❌ *Ошибка при очистке*", parse_mode="Markdown")
